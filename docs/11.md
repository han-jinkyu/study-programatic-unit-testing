...
- `assertFalse(search.errored())`는 세번째 테스트로 넘긴다.
...
- 각 테스트가 분명하게 답할 수 있는 가장 큰 질문은 `"왜 그런 결과를 기대하는가?"`다.
- `테스트 준비`와 `단언 부분`을 서로 `연관` 지을 수 있어야 한다.
- `returnsMatchesShowingContextWhenSearchStringInContent 메서드`의 테스트 데이터는 매우 길다. 테스트를 읽는 사람은 테스트를 이해하기 위해 노력해야 한다.
- 메서드는 결국 특정 구절이 나오는 곳에서 앞뒤 10글자를 계산한다. 따라서 좀더 쉽게 알아볼 수 있는 구절과 단어를 적는 것이 좋다.
...
- `noMatchesReturnedWhenSearchStringNotInContent 메서드`는 실제 URL 입력 스트림을 사용하기 때문에 느리다.
- 임의의 텍스트를 넣어 stream 필드를 초기화한다.
- 또한 이해하기 쉽도록 검색어를 "text that doesn't match"로 넣는다.
...
- 잘못 만든 테스트를 깔끔하게 분할하면서 다른 테스트를 추가하기 쉬워졌다.
- errored() 질의에 true를 반환하는 테스트를 작성해 본다. 
- 반대 테스트도 작성해 본다.
